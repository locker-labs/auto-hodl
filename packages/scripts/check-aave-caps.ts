...
const main = async () => {
    ...
    console.log('Asset'.padEnd(8) + 'Supply Cap'.padEnd(15) + 'Current Supply'.padEnd(18) + 'Available'.padEnd(15) + 'Utilization'.padEnd(12) + 'Status');
    ...
    for (const [symbol, { address, decimals }] of Object.entries(ASSETS)) {
        try {
            ...
            if (!configData[8]) { // isActive
                status = '⚫ Inactive';
            }

            if (configData[9]) { // isFrozen
                status = '🔵 Frozen';
            }
    ...
    const availableAssets = results.filter(r =>
        r.isActive &&
        !r.isFrozen &&
    ...
    if (availableAssets.length === 0) {
        ...
        availableAssets
            ...
            .forEach(asset => {
                console.log(`   ${asset.symbol}: ${asset.available === 'unlimited' ? 'No supply cap' : `${asset.available} available`}`);
            });
    }
};
