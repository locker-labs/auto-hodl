...
const main = async () => {
  ...
  if (tokenBalance < amount) {
    throw new Error(
      `Insufficient token balance. Have: ${Number(tokenBalance) / decimals} token, Need: ${Number(amount) / decimals} token`,
    );
  }
  ...
  if (amount > maxDeposit) {
    console.log(
      `⚠️ Deposit amount (${Number(amount) / decimals} token) exceeds maximum allowed (${Number(maxDeposit) / decimals} token)`,
    );
