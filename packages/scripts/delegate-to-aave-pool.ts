...
// Delegation redemption setup for Aave pool supply
function setupAaveDelegationRedemption(signedDelegation: any) {
  ...
  const AAVE_POOL = '0x6Ae43d3271ff6888e7Fc43Fd7321a503ff738951'; // Aave Pool address
  const SUPPLY_AMOUNT = parseUnits('1', 18); // 1 AAVE token (18 decimals)

  // Step 1: Approve AAVE token to Aave pool
  const approveCalldata = encodeFunctionData({
    ...
    args: [AAVE_POOL as Address, SUPPLY_AMOUNT],
  });
  ...
  const supplyCalldata = encodeFunctionData({
    ...
    functionName: 'supply',
    args: [AAVE_TOKEN as Address, SUPPLY_AMOUNT, signedDelegation.delegator as Address, 0], // Supply to smart account
  });
...
function logSummary(
  ...
  if (transferType === 'AAVE') {
    console.log(`   - Action Type: Aave Token Approval + Pool Supply`);
    ...
    console.log(`   - Operations: 1) Approve AAVE to pool, 2) Supply AAVE to pool`);
  } else {
...
const executeScript = async (transferType: 'ETH' | 'ERC20' | 'AAVE' = 'ETH') => {
  console.log(
    `🚀 Starting DTK delegation script (${transferType === 'AAVE' ? 'Aave Token Approval + Pool Supply' : transferType + ' Transfer'})...`,
  );
  ...
  try {
    ...
    const { publicClient } = setupClients(config, delegatorEoa);
    ...
    const transactionHash = await executeDelegationRedemption(config, delegateEoa, delegations, mode, executions);
